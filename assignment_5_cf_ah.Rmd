...
### Data and Methods  
The data used in this report was collected by the H.J. Andrews Experimental Forest Long-Term Ecological Research (LTER) program. Data collection for Pacific Giant Salamanders began in 1993. The main location site is the Mack Creek, which is then divided into areas: clear cut and old growth. 500 year old coniferous forests are located in old growth areas. The area of clear cut and old growth sections are roughly 150 m reaches of stream. For more information on the sites or variables, see the [metadata](https://portal.edirepository.org/nis/metadataviewer?packageid=knb-lter-and.4027.12). The statistical tests performed were a chi-squared test, ... The significance level used was 0.05. R Studio Version 1.2.5001 was used to perform all tests and to produce this report.
...
**Table 1:** 
Pacific Giant Salamander counts in channel classifications and area sections in 2017. The channel classifications are cascade, pool, and side channel. The area sections are clear cut and old growth. Proportions are in parentheses. Data: Gregory S. V. 2016.  
...
```
Most Pacific Giant Salamander are found in the cascade channel for both area sections ($\sf{n_{cc}}$ = `r round(chi_test$observed[1],2)`, $\sf{n_{OG}}$ = `r round(chi_test$observed[2],2)`). This is followed by the side ($\sf{n_{cc}}$ = `r round(chi_test$observed[5],2)`, $\sf{n_{OG}}$ = `r round(chi_test$observed[6],2)`) and the pool channels ($\sf{n_{cc}}$ = `r round(chi_test$observed[3],2)`, $\sf{n_{OG}}$ = `r round(chi_test$observed[4],2)`) (See Table 1). There is not evidence to suggest that there is a significant effect of area sections, old growth and clear cut, on channel classification, cascade, pool, and side channel ($\chi$^2^(`r chi_test$parameter`) = `r round(chi_test$statistic,4)`, *p* = `r round(chi_test$p.value, 2)`).
